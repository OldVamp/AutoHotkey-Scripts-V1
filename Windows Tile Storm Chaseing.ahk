...
^+1:: ; ctrl + shift + 1
    OpenTileWindowsOVMathGrid(YouTubers,ScreenGrids)
    Return
...
OpenTileWindowsOVMathGrid(ChannelList, GridsToUse)
{
     Loop, parse, ChannelList, `, ,%A_Space%`r`n""
        ++WindowCount
    ; number of windows coutned
    ...
    Loop, parse, ChannelList, `, ,%A_Space%`r`n""
    {
        ...
        ; bring window to the front focus
        WinActivate, ahk_pid %OpenedWindow%
        ...
        wpy := Window_Pos_Y - 0
        ; move windows
        ; WinMove, ahk_pid %OpenedWindow%,, %Window_Pos_X%, %Window_Pos_Y%, %WindowSize_X%, %WindowSize_Y%
        WinMove, ahk_pid %OpenedWindow%,, %wpx%, %wpy%, %wsx%, %wsy%
        
    } ; End parse-loop
    return WindowCount ; N windows were tiled
} ; OpenTileWindowsOVMathGrid
